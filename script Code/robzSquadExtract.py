...
def extractBreedData(breedFiles):
    ...
    breedInventory = []
    breedDictionary = {"filePath":[], "country":[], "breedName":[],"health":[], "finalSkill":[], "veterancy":[],
    ...
    for breedFile in breedFiles:
        ...
        breedDictionary["filePath"].append(breedFile) 
...
def deleteFiles(unusedBreeds):
    breedDirectoryD = ["./resource/gamelogic/set/breed/mp/jap/",
                       "./resource/gamelogic/set/breed/mp/eng/",
                       "./resource/gamelogic/set/breed/mp/rus/",
                       "./resource/gamelogic/set/breed/mp/ger/",
                       "./resource/gamelogic/set/breed/mp/ger2/",
                       "./resource/gamelogic/set/breed/mp/ger_ss/",
                       "./resource/gamelogic/set/breed/mp/usa/",
                       "./resource/gamelogic/set/breed/mp/rus_guard/"
    ]
...
#-----------------------------------------------------------------------------------------------------------------------------
def userInputDelete():
    userInput = input("Type 'y' to delete any unused breeds or press enter to ignore")
    if userInput == 'y':
        answer = True
    else:
        answer = False
...
def get_file_path(default_path):
    ...
    user_input = input(f"Enter file path for '{default_path}' (or press Enter to use default):  Current working directory:" + os.getcwd()).strip()
...
def main():
    ...
    deleteUnsedBreeds = userInputDelete()

    #os.chdir(getUserInput())
    breedDirectory =  get_file_path("./resource/gamelogic/set/breed/mp")
    breedFiles = getBreedFiles(breedDirectory)

    breedDictionary = extractBreedData(breedFiles)
    #print(breedDictionary)

    soldierDirectory = get_file_path("./resource/gamelogic/set/multiplayer/units/soldiers.set")
    ...
    squadArray = []
    squadDirectory = get_file_path("./resource/gamelogic/set/multiplayer/units")
    ...
    veterancyDirectory = "./resource/gamelogic/set/ability/veterancy.set"
    ...
    if deleteUnsedBreeds == True:
        deleteFiles(unusedBreeds)
        print("Files Deleted")


    data_frame = pd.DataFrame(squadDictionary)
    data_frame.to_csv('RobzSquads.csv', index=False)
    print('DataFrame has been written to Excel File successfully.')
